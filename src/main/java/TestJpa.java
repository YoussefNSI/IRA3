...
private static void findLivreById(EntityManager em, Long id) {
    ...
    if (livre != null) {
        IO.println("Livre trouvé : " + livre.getTitre() + " - " + livre.getAuteur());
    } else {
        IO.println("Livre non trouvé (id=" + id + ")");
    }
}
...
private static void updateLivreTitre(EntityManager em, Long id, String nouveauTitre) {
    ...
    try {
        ...
        if (livre != null) {
            ...
            em.getTransaction().commit();
            IO.println("Titre modifié : " + nouveauTitre);
        } else {
            em.getTransaction().rollback();
            IO.println("Livre non trouvé (id=" + id + ")");
        }
...
private static void findLivreByTitre(EntityManager em, String titre) {
    ...
    if (!livres.isEmpty()) {
        IO.println("Livres trouvés (titre=" + titre + ") : " + livres.size());
        livres.forEach(System.out::println);
    } else {
        IO.println("Aucun livre trouvé (titre=" + titre + ")");
    }
}
...
private static void findLivresByAuteur(EntityManager em, String auteur) {
    ...
    if (!livres.isEmpty()) {
        IO.println("Livres de " + auteur + " : " + livres.size());
        livres.forEach(l -> IO.println("  " + l.getTitre()));
    } else {
        IO.println("Aucun livre trouvé (auteur=" + auteur + ")");
    }
}
...
private static void deleteLivre(EntityManager em, Long id) {
    ...
    try {
        ...
        if (livre != null) {
            em.remove(livre);
            em.getTransaction().commit();
            IO.println("Livre supprimé : " + livre);
        } else {
            em.getTransaction().rollback();
            IO.println("Livre non trouvé (id=" + id + ")");
        }
...
private static void afficherTousLesLivres(EntityManager em) {
    ...
    IO.println("Total livres : " + livres.size());
    livres.forEach(l -> IO.println("  [" + l.getId() + "] " + l.getTitre() + " - " + l.getAuteur()));
}
